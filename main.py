...
def main():
    ...
    df = pd.DataFrame(
        evecs, columns=[f"Eigenvector-{i}" for i in range(n_evecs)], index=gene_list
    )
    ...
    df = pd.DataFrame(
        ...
        index=[f"Eigenvector-{i}" for i in range(n_evecs)],
    )
    ...
    json.dump(
        gt.disease_module_indices, open(f"{folder}/disease_module_indices.json", "w")
    )
    json.dump(
        [[str(v) for v in w] for w in all_patient_mutations],
        open(f"{folder}/all_patient_mutations.json", "w"),
    )
...
class PatientInteractionMatrix:
    ...
    def __init__(self, nodelist, mutations):
        ...
        self.gene_pair_indices = [
            (node_to_index[gene1], node_to_index[gene2])
...
class GroundTruth:
    ...
    def __init__(self, module_size, n_modules, total_nodes):
        ...
        self.modules = [
            list(range(module_size * n, module_size * (n + 1)))
...
class PPI:
    ...
    def __init__(self, gt, p_inter, p_intra):
