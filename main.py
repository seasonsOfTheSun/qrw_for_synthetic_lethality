...
def main():
    ...
    # we consider an arbitrary network genes, and some number of gene modules of constant size
    ...
    # we select two f the gene modules to be the disease modules 
...
class DataInteractionMatrix:
    ...
    def __init__(self,nodelist,mutations,disease_statuses,patient_weighting_map=None):
        ...
        sum_{patients} sum_{ij} (|i> \tensor |j>)(<i| \tensor <j|)
        where i,j are all pairs of genes in a particual patient,
        summed over all the patients selected
